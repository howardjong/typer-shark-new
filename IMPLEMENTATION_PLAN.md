...
| E1 | Key Camp | Add posture/finger/letter/word drills, keyboard/hands guide, repeat/skip/slow controls, and `;` acceptance only in this mode. | Lesson sequence and input-policy tests; keyboard-only manual pass. | `feat: add key camp tutor` | Next |
